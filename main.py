...
def get_connection():
    # Streamlit Cloud üzerindeki Secrets kısmına eklediğiniz DATABASE_URL'i kullanır
    return psycopg2.connect(st.secrets["DATABASE_URL"])

def init_db():
    conn = get_connection()
    cur = conn.cursor()
    # Öğrenciler Tablosu (TC UNIQUE ile mükerrer kayıt engellenir)
    cur.execute("""
...
st.title("🚀 Güvenli Tahsilat Sistemi (Bulut)")
        ...
        if st.button("Tahsilatı Onayla"):
            ...
            try:
                if tutar_giris < asil_tutar:
                    # Kısmi ödeme: Mevcut olanı güncelle, kalanı yeni taksit yap
